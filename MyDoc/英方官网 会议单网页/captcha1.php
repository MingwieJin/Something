<?php
  ...
  session_start();
  $image = imagecreatetruecolor(150, 50);    //1>设置验证码图片大小的函数
...
//    imagestring($image,$fontsize,$x,$y,$fontcontent,$fontcolor);
  ...
  //10>存到session
  $_SESSION['authcode1'] = $captcha_code;
  //8>增加干扰元素，设置雪花点
  for($i=0;$i<200;$i++){
    //设置点的颜色，50-200颜色比数字浅，不干扰阅读
    $pointcolor = imagecolorallocate($image,rand(50,200), rand(50,200), rand(50,200));    
    //imagesetpixel — 画一个单一像素
    imagesetpixel($image, rand(1,149), rand(1,49), $pointcolor);
  }
  //9>增加干扰元素，设置横线
  for($i=0;$i<6;$i++){
    //设置线的颜色
    ...
    //设置线，两点一线
    imageline($image,rand(1,149), rand(1,49),rand(1,149), rand(1,49),$linecolor);
  }
